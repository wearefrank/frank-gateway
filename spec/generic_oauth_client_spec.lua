describe("generic-oauth-client plugin", function()
	local plugin
	...
	before_each(function()
		...
		package.loaded["apisix.plugins.generic-oauth-client"] = nil
			...
			return {
				...
				json = {
					decode = function(body)
						-- Minimal JSON decoder for expected test shapes only
		...
		-- Stub the shared memory token cache used at module level
		_G.ngx = {
			shared = {
				["generic-oauth-client-cache"] = {
			...
			escape_uri = function(value)
				...
				return value -- identity is fine for unit tests
			end,
		}

		plugin = require("apisix.plugins.generic-oauth-client")
	end)
	...
	it("connects to IDP with parsed URL parts", function()
		local conf = {
			client_id_field_name = "client_id",
			client_id_value = "my-client",
			token_endpoint = "http://idp:9081/token",
	...
	it("sends client_id as form-encoded body field", function()
		local conf = {
			client_id_field_name = "client_id",
			client_id_value = "my-client",
			token_endpoint = "http://idp:9081/token",
	...
	it("appends custom_parameters to the request body", function()
		local conf = {
			client_id_field_name = "client_id",
			client_id_value = "my-client",
			token_endpoint = "http://idp:9081/token",
			default_expiration = 300,
			custom_parameters = { grant_type = "client_credentials", scope = "api" },
		}
		local ctx = {}
		...
		assert.is_not_nil(string.find(httpc_request_args.body, "scope=api"))
	end)

	it("stores the fetched token in cache with the expires_in TTL", function()
	...
	it("falls back to default_expiration when IDP response omits expires_in", function()
		...
		-- Token should be cached; TTL is opaque here but no error should occur
		assert.are.same("no-expiry-tok", cache_store["my-client"])
		assert.are.same(0, #error_logs)
	end)

	it("returns 500 and logs error when connect fails", function()
