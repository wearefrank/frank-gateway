## Frank-Gateway (APISIX standalone)

Frank!Gateway adds plugins to Apache APISIX and is deployed by this chart in standalone mode.

based on:
Apache APISIX is a dynamic, real-time, high-performance API gateway.
APISIX provides rich traffic management features such as load balancing, dynamic upstream, canary release, circuit breaking, authentication, observability, and more.

This chart bootstraps Frank Gateway on a Kubernetes cluster using [Helm](https://helm.sh).
...
To install the chart with the release name `my-frankgateway`:

```sh
helm install [RELEASE_NAME] ./charts/frankgateway --namespace ingress-apisix --create-namespace
```

By default, the chart uses the image `frank-api-gateway:latest`.

To install with a realistic standalone Frank Gateway route/plugin example:

```sh
helm install [RELEASE_NAME] ./charts/frankgateway \
	--namespace ingress-apisix \
	--create-namespace \
	-f ./charts/frankgateway/values.standalone-example.yaml
```

## Uninstall

 To uninstall/delete a Helm release `my-frankgateway`:
